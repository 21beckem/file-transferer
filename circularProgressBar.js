class CircularProgressBar {
    constructor(options) {
      ...
      const { pie, percent, color, strokeWidth, opacity, number, size, fontSize, fontWeight, fontColor, fractionTotal } = this.options;
      this.pie = pie;
      ...
      this.fractionTotal = fractionTotal || false;
      this.end = 264;
  
      this.createSvg();
    }

    updateProgress(percent, total = this.fractionTotal) {
      this.fractionTotal = total;
    ...
    circularProgressBar() {
      ...
      this.percentElement();
      const d = this.end * this.percent / 100;
    ...
    percentElement() {
      ...
      percent.setAttribute('style', `white-space: nowrap; position: absolute; left: 50%; top: 50%;transform: translate(-50%, -50%); font-size: ${this.fontSize}; font-weight: ${this.fontWeight}; color: ${this.fontColor}`);
    ...
    percentElementUpdate(numbers) {
      ...
      if (this.fractionTotal) {
        percentNumber.innerHTML = `${((numbers / 100) * this.fractionTotal)} / ${this.fractionTotal}`;
      } else {
